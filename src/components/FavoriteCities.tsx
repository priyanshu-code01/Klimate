...
const FavoriteCities = () => {
  ...
  return (
    <>
      <h1 className="text-xl font-bold tracking-tight">Favorite</h1>
...
function FavoriteCityTablet({
  ...
  return (
    ...
    >
      <Button variant={'ghost'} size={'icon'} className='absolute right-1 top-1 h-6 w-6 rounded-full p-0 hover:text-destructive-foreground group-hover:opacity-100' onClick={(e) => { e.stopPropagation(); onRemove(id); toast.error(`Removed ${name} from Favorites`) }}>
        <X className="h-4 w-4" />
      </Button>
